  ...
  for (Module::iterator MI = M.begin(), ME = M.end(); MI != ME; ++MI) {
    ...
    while(Worklist.size()) {
      ...
      for (BasicBlock::iterator bi = BB->begin(); bi != BB->end(); ) {
        ...
        if(hoist) {
          CI->removeFromParent();
          auto Term = L->getLoopPreheader()->getTerminator();
          auto Found = std::find_if(L->getLoopPreheader()->getInstList().begin(),
                                    L->getLoopPreheader()->getInstList().end(),
                                    [&](const Instruction &i) {
                                      if (&i == Term)
                                        return true;
                                      return false;
                                    });
          L->getLoopPreheader()->getInstList().insert(Found, CI);
        }
      }
    }
  }
